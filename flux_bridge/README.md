...
Republishes flux channels as DDS topics so the stock ROS 2 tools can read them. One node, started by hand, that relays a channel only while a ROS 2 subscriber for its name exists. Recording is not this path: `flux_bag` writes a channel to a bag straight from shared memory. The user document is [`docs/en/bridge.en.md`](../docs/en/bridge.en.md).
...
| `flux_bridge/verb/` | `ros2 topic echo_flux`, `ros2 topic hz_flux` |
