...
def _cpp_to_frame(block, out, ind, msg, bld, depth):
    for p in block.placed:
        ...
        if p.kind == LeafKind.FIXED and p.count == 1:
            ...
            i = f"i{depth}"
            count = "" if p.count != DYNAMIC_COUNT else f"{src}.size()"
            out.append(f"{ind}{bld}.alloc__{n}({count});")
            ...
            out.append(f"{ind}{{")
            count = "" if p.count != DYNAMIC_COUNT else f"{src}.size()"
            out.append(f"{ind}  auto {arr} = {bld}.alloc__{n}({count});")
            out.append(f"{ind}  for (std::size_t {i} = 0; {i} < {src}.size(); ++{i}) {{")
...
{chr(10).join(to_msg)}
  if (!v.ok__()) {{
    throw std::runtime_error("flux: {layout.type_name}: the frame does not match its schema");
  }}
}}
...
def _py_to_frame(block, out, ind, msg, bld, depth):
    for p in block.placed:
        ...
        if p.kind == LeafKind.FIXED and p.count == 1:
            ...
            count = "" if p.count != DYNAMIC_COUNT else f"len({src_v})"
            out.append(f"{ind}{arr} = {bld}.alloc__{n}({count})")
...
def _py_to_msg(block, out, ind, msg, view, depth, alias, tn):
    for p in block.placed:
        ...
        if p.kind == LeafKind.STRING_ARRAY and not p.is_dynamic:
            ...
            # time, after rclpy's own setter (checks off by default) has let it through.
            out.append(f"{ind}{dst} = {view}.{n}.item()")
