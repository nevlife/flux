...
def _builder_body(block, base, out, prefix=(), ind="    "):
    ...
    for p in block.placed:
        ...
        if p.kind == LeafKind.FIXED and p.count == 1:
            out.append(f"{ind}def _set__{n}(self, v):")
            out.append(f"{ind}    self._w.put({at(o)}, {np_type(p.dtype)}, v)")
            out.append(f"{ind}{n} = property(fset=_set__{n})")
            ...
            if p.kind == LeafKind.HEADER:
                out.append("")
                out.append(f"{ind}def _set__{n}__frame_id(self, s):")
                out.append(f"{ind}    self._w.put_str({at(p.frame_id_offset)}, s)")
                out.append(f"{ind}{n}__frame_id = property(fset=_set__{n}__frame_id)")
        else:  # RECORD_COLUMN / JAGGED
            cls = _elem_class(prefix + (p,))
            # A fixed T[N] takes no count, as in C++: N is the schema's.
            fixed = p.count != DYNAMIC_COUNT
            out.append(f"{ind}def alloc__{n}(self{'' if fixed else ', n'}):")
            if fixed:
                out.append(f"{ind}    n = {p.count}")
...
from flux_gen.wire import ElemArray, ElemSeq, NullWriter, Reader, Writer
...
class Builder:
    ...
    def __init__(self, buf, loan=None):
        # No buffer is a build__ that found no free slot: writes go nowhere, as in C++.
        self._w = Writer(buf, SCALAR_BYTES__) if buf is not None else NullWriter()
        self._loan = loan

    def __bool__(self):
        """False with no slot or once a field did not fit; commit__() then refuses."""
        return not self._w.bad
...
{chr(10).join(builder)}
    ...
    def commit__(self):
        ...
        if self._loan is None:
            # Imported here rather than at module domain so a Builder over a plain bytearray
            # needs no flux runtime at all.
            from flux import Published

            if isinstance(self._w, NullWriter):
                return Published.BACKPRESSURE  # the loan never happened
            return Published.TOO_LARGE  # a caller's own buffer: nothing to publish
        if self._w.bad:
            from flux import Published

            return Published.TOO_LARGE  # a write failed: the frame is torn, as C++ refuses it
        return self._loan.commit(nbytes=self._w.size)


def build__(publisher):
    """Loan a slot from `publisher` and return a Builder over it. With no free slot the Builder
    is false, writes into it go nowhere, and commit__() returns BACKPRESSURE, as in C++."""
    loan = publisher.loan(publisher.slot_size, dtype="uint8")
    return Builder(None) if loan is None else Builder(loan.array, loan)
