...
{
  // dtype_size is what a consumer sizes its view from, so every DType must answer with its real
  // width. A value this build does not know answers 0, which is what makes an unknown dtype a
  // refused frame rather than a view sized from a guess.
