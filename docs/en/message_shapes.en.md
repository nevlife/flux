...
The receiving side assumes another process wrote the frame and trusts none of the descriptors. It checks every range and alignment. On a mismatch, C++ latches `ok__()` to false and Python raises `WireError`. There is no path that hands out a pointer outside the frame. String bytes are not checked for UTF-8 in C++, where a string is a `std::string_view` of the bytes as rclcpp keeps a `std::string`. Python has to decode them, and a string that is not UTF-8 raises `WireError` like any other unreadable frame.

In `frame_to_msg`, if the length of an array in the frame differs from the schema's `T[N]`, both languages raise an explicit error. C++ raises `std::length_error`, Python raises `ValueError`. The length is data the frame carried and N is the schema. A truncated or padded copy would be a silently wrong message. For the same reason, C++ `frame_to_msg` throws `std::runtime_error` on a frame whose descriptors do not fit, and Python raises `WireError`.

The write side enforces the same length. A fixed `T[N]` array of strings (C++) or of records (both languages) is allocated with `alloc__x()`, which takes no count, so a wrong count cannot be written: in C++ it is a compile error and in Python a `TypeError`. In Python a string list of the wrong length assigned to a `T[N]` field raises `ValueError`. The writer latches before it raises, so `commit__()` returns `TooLarge` even when the caller caught the error. The same holds for a write that does not fit the slot. A wrong frame is never produced. The check in `frame_to_msg` is the last line of defense against frames produced by a foreign (external or older) writer.
